...
func OldVersionCheck() (bool, string) {
	...
	if err != nil {
		// 不知道的版本，就认为是支持的
		showNoticeBox("版本确认提示", "余烬无法获取您的操作系统版本，请确认正在使用 Windows 10/Windows Server 2016 或更高版本的 Windows。")
	...
	if (WINXP <= os) && (os < WIN10) {
		// 展示提示弹窗，提示用户升级
		showMsgBox("版本升级提示", fmt.Sprintf("您的操作系统版本「%s」过旧，余烬未来将不再支持，请尽快升级系统至 Windows 10/Windows Server 2016 或更高版本。", osNameMap[os]))
