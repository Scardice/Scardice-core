...
- **幂等 / 去重**：每个迁移应用前先问 `Store.IsApplied(id)`；`JSONStore` 以最后一次尝试的成功状态判断是否跳过。
- **失败处理**：任意迁移返回错误时，`ApplyAll` 立即中止，并把错误向上抛（“因无法忽略的错误，升级 X 失败”）。声明为幂等的迁移可在下次启动重试；非幂等迁移存在失败记录时会拒绝自动重试。
- **记录**：无论成功失败，都会向 `upgrade_metadata.json` 原子追加一条 `UpgradeRecord`（含时间、成功标志、日志），保留完整尝试历史。
...
测试位于 `migrate/v2/*_test.go`、`migrate/v2/v160/*_test.go`，以及 `utils/upgrader/store/json_store_test.go`：

- `json_store_test.go`：JSONStore 的原子追加、失败尝试持久化、重开后重试及最终成功状态。
...
- `full_flow_test.go`：用 `testdata/full_setup_logs.sql` + `testdata/full_setup_data.sql` 造假库，跑完整迁移链路并断言结果，再跑第二次验证幂等。
