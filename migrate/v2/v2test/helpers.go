...
import (
	...
	engine "Scardice-core/utils/dboperator/engine"
...
// NewTestManager 复刻 enter.go 的注册顺序，升级记录存入测试临时目录。
func NewTestManager(t *testing.T, op engine.DatabaseOperator) *upgrade.Manager {
	t.Helper()
	storer := store.NewJSONStore(filepath.Join(t.TempDir(), "upgrade_metadata.json"))
