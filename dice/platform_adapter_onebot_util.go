...
func (p *PlatformAdapterOnebot) extractServePort() string {
	addr := ""
	if p.EndPoint != nil && p.EndPoint.Session != nil && p.EndPoint.Session.Parent != nil && p.EndPoint.Session.Parent.Parent != nil {
		addr = p.EndPoint.Session.Parent.Parent.ServeAddress
	}
	if addr == "" {
		return "3211"
	}
...
func (p *PlatformAdapterOnebot) resolveLocalAsset(rel string) (string, error) {
	...
	token := ""
	if p.EndPoint != nil && p.EndPoint.Session != nil && p.EndPoint.Session.Parent != nil && p.EndPoint.Session.Parent.Parent != nil {
		token = p.EndPoint.Session.Parent.Parent.AssetImageToken
	}
