...
func TestParseAtInfoPreservesQualifiedIDAndRobotFlag(t *testing.T) {
	cmdArgs := new(CmdArgs)
	msg := &Message{
		Platform: "QQ",
	...
	parseAtInfo(cmdArgs, msg, "QQ:100000")

	if len(cmdArgs.At) != 1 {
		t.Fatalf("mention count = %d, want 1", len(cmdArgs.At))
	}
	if cmdArgs.At[0].UserID != "QQ:4010000000" {
		t.Fatalf("mention user ID = %q, want %q", cmdArgs.At[0].UserID, "QQ:4010000000")
	}
	if !cmdArgs.At[0].IsRobot {
		t.Fatal("mention robot flag was not preserved")
	}
}
