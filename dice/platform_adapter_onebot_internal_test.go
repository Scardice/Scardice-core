...
func (m *onebotTestEmitter) GetMsg(context.Context, int64) (*emitterTypes.GetMsgRes, error) {
	return &emitterTypes.GetMsgRes{}, nil
}

func (m *onebotTestEmitter) DelMsg(context.Context, int64) error { return nil }
...
func TestCheckBotGroupRoleOnebotBypassesCache(t *testing.T) {
	...
	pa := &PlatformAdapterOnebot{ctx: t.Context(), sendEmitter: em}
	ctx := &MsgContext{
	...
	// Then
	if !ok || role != "admin" {
		t.Fatalf("bot group role = (%q, %v), want (admin, true)", role, ok)
	}
	if len(em.groupMemberInfoCalls) != 1 {
		t.Fatalf("group member info calls = %d, want 1", len(em.groupMemberInfoCalls))
	}
	call := em.groupMemberInfoCalls[0]
	if call.GroupID != 2010 || call.UserID != 10010 || !call.NoCache {
		t.Fatalf("group member info call = %#v, want cache bypass", call)
	}
}
